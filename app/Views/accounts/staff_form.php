...
<?php echo form_open_multipart($form_link, array('id'=>'bb_ajax_form', 'class'=>'')); ?>
    ...
    <?php if($param2 == 'edit' || $param2 == '') { ?>
        ...
        <div class="row">
            ...
            <?php if($role == 'developer' || $role == 'administrator' || $role == 'partner') {?>
            <div class="col-sm-6">
                <div class="form-group">
            ...
            <?php if(!empty($e_branch_id)) {?>
                <div class="col-sm-6">
                    <div class="form-group" id="branch_resp">
                        <label for="activate">Branch</label>
                        <select class="form-select js-select2" data-search="on" id="branch" name="branch" required>
                            <option value=" ">Select</option>
                            <?php $cat = $this->Crud->read_single_order('city_id', $e_lga_id, 'branch', 'name', 'asc');foreach ($cat as $ca) {?>
                                    <option value="<?=$ca->id;?>" <?php if(!empty($e_branch_id)){if($e_branch_id == $ca->id){echo 'selected';}} ?>><?=ucwords($ca->name); ?></option>
                                <?php }?>
                        </select>
                    </div>
                </div>
            ...
            <?php } ?>
            <?php } else { ?>
                <input type="hidden" class="form-control" name="country_id" id="country_id"  value="<?php if(!empty($e_country_id)){echo $e_country_id;} else {echo $this->Crud->read_field('id', $log_id, 'user', 'country_id');} ?>">
                <input type="hidden" class="form-control" name="state" id="state"  value="<?php if(!empty($state_id)){echo $state_id;} else {echo $this->Crud->read_field('id', $log_id, 'user', 'state_id');} ?>">
                <input type="hidden" class="form-control" name="lga" id="lga"  value="<?php if(!empty($e_lga_id)){echo $e_lga_id;} else {echo $this->Crud->read_field('id', $log_id, 'user', 'lga_id');} ?>">
                <input type="hidden" class="form-control" name="branch" id="branch"  value="<?php if(!empty($e_branch_id)){echo $e_branch_id;} else {echo $this->Crud->read_field('id', $log_id, 'user', 'branch_id');} ?>">
            <?php } ?>
            <div class="col-sm-6">
